...
const FIXINGS = ["rigged", "freestanding", "wall_mounted", "shell_mounted", "floor", "digital", "other"];
    ...
    <form onSubmit={onSubmit} className="grid max-w-4xl gap-5">
      <section className="grid gap-3 sm:grid-cols-2">
        ...
        <div className="space-y-1.5">
          ...
          <SelectNative id="ownerRole" name="ownerRole" defaultValue={values.ownerRole ?? "ops"}>
      ...
      <section className="grid gap-3 sm:grid-cols-3">
        ...
        <div className="space-y-1.5">
          ...
          <Input id="heightMm" name="heightMm" type="number" min="1" defaultValue={values.heightMm ?? ""} />
        </div>
        <div className="space-y-1.5">
          <Label htmlFor="depthMm">Depth (mm)</Label>
          <Input id="depthMm" name="depthMm" type="number" min="1" defaultValue={values.depthMm ?? ""} />
        </div>
        <div className="space-y-1.5">
          <Label htmlFor="quantity">Quantity</Label>
          <Input id="quantity" name="quantity" type="number" min="1" defaultValue={values.quantity ?? 1} />
        </div>
        <div className="space-y-1.5">
        ...
        <div className="space-y-1.5">
          ...
          <SelectNative id="fixingMethod" name="fixingMethod" defaultValue={values.fixingMethod ?? ""}>
            <option value="">— Select —</option>
            {FIXINGS.map((f) => (
        ...
        <div className="space-y-1.5">
          ...
          <Input id="weightKg" name="weightKg" type="number" step="0.1" min="0" defaultValue={values.weightKg ?? ""} />
        </div>
      </section>
      ...
      {canSeeCosts && (
        <details open={costsOpen} className="rounded-lg border">
          ...
          <div className="space-y-1.5">
            ...
            <Input id="costEstimate" name="costEstimate" type="number" min="0" step="0.01" defaultValue={values.costEstimate ?? ""} disabled={!canEditCosts} />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="costActual">Cost actual (£)</Label>
            <Input id="costActual" name="costActual" type="number" min="0" step="0.01" defaultValue={values.costActual ?? ""} disabled={!canEditCosts} />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="poNumber">PO number</Label>
            <Input id="poNumber" name="poNumber" defaultValue={values.poNumber ?? ""} disabled={!canEditCosts} />
          </div>
          <div className="space-y-1.5">
            <Label htmlFor="supplierId">Supplier</Label>
            <SelectNative id="supplierId" name="supplierId" defaultValue={values.supplierId ?? ""} disabled={!canEditCosts}>
              <option value="">— None —</option>
      ...
      <details open={datesOpen} className="rounded-lg border">
        ...
        <div className="space-y-1.5">
          ...
          <Input id="printDeadline" name="printDeadline" type="date" defaultValue={values.printDeadline ?? ""} />
        </div>
        <div className="space-y-1.5">
