...
- **Accounts with passwords** — everyone signs in with their own account. Invitations (team members and approvers) are emailed; the link opens a page to set up a profile and choose a password. "Forgot password" emails a reset link; a sign-in link is still available.
- **Shows** — one form for a new show: name, logo, series, venue and address, dates.
- **Suppliers** — a directory of companies with what each does (signage print, screens, staffing…), filterable; the supplier column in the schedule shows who's making what.
...
- **Sponsorship** — everything sold to sponsors, by sponsor, reusing the same sign-off and artwork versioning.
- **My Work** — every user's personal to-do list (self-created or assigned by colleagues, with due dates and notifications), with a link to the sign-offs waiting on them.
...
- **Tests** — 565 unit tests (authz + override matrix, status machines, workflow engine, department routing and ordering, deadlines, exports and labels, DB-backed ref concurrency/RLS/audit-trigger/cron idempotency/staff invitations/departments) and 30 Playwright end-to-end tests (full signage lifecycle to installed with a photo, three-department sign-off with a named approver, adding a department and approver who sets up an account and signs off, sponsor signage, suppliers and services, new show with logo, exports, partner item page, team, phone layouts).
